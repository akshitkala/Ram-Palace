...
const MiniGallery = () => {
  ...
  return <section ref={containerRef} className="w-full bg-[#fdfbf7] py-20 overflow-hidden">
      <div className="max-w-7xl mx-auto mb-16 px-6 text-center">
        <h2 className="font-heading text-4xl md:text-5xl text-[#2a2015] mb-4">{content.minigallery.mini_gallery_heading}</h2>
        <p className="font-body text-[#8c7b6c] text-lg tracking-wide">{content.minigallery.mini_gallery_subtext}</p>
      </div>

      {/* Marquee Container */}
      <div className="w-full overflow-hidden flex">
        {/* Track */}
        <div ref={trackRef} className="flex gap-14 md:gap-20 px-4 md:px-0 w-max will-change-transform">
          {images.map((img, index) => <div key={`${img.id}-${index}`} className={`
              ...
              <div className="relative overflow-hidden rounded-xl shadow-lg lg:h-[50vh] h-[30vh] gallery-item">
                <img src={img.image} alt={img.alt} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" loading="lazy" />
