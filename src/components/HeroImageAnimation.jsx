...
const HeroImageAnimation = () => {
  ...
  return <div ref={containerRef} className="absolute inset-0 w-full h-full overflow-hidden">
      {/* Background (static) */}
      <img src={bg} alt={content.heroimageanimation.hero_bg_image_alt} className="absolute inset-0 w-full h-full object-cover brightness-110 bg-image" />

      {/* Palace (animated from bottom) */}
      <img src={palace} alt={content.heroimageanimation.hero_palace_image_alt} className="absolute inset-0 w-full h-full object-cover palace translate-y-full brightness-110" />
    </div>;
};
