// ─────────────────────────────────────────────────────────────────────────────
// GD Foods India — Catering Page Data
// Sourced from: Basti Ram Palace Company Profile Brochure
// ─────────────────────────────────────────────────────────────────────────────

// ── 1. HERO ──────────────────────────────────────────────────────────────────
export const cateringHero = {
  image: "/images/hero/Catering.webp",
  headline: "Where Flavour Meets Precision",
  description:
    "GD Foods India is a trusted catering partner delivering flavorful, hygienic, and thoughtfully crafted meals for corporate offices, institutions, and special events.",
  subtext:
    "Our commitment to consistency, customization, and timely delivery makes us a preferred choice for catering that truly cares.",
};

// ── 2. CULINARY PHILOSOPHY ────────────────────────────────────────────────────
export const culinaryPhilosophy = {
  image: "/images/gallery/vadim-paripa-PuXtB1B4zL8-unsplash.webp", 
  heading: "Food Crafted with Responsibility",
  tagline: "Every dish prepared with precision. Every event handled with care.",
  description:
    "At GD Foods India, food is more than a service — it's a responsibility we take seriously. Every menu is planned with care, every dish prepared with precision, and every event handled with attention to detail. We believe great catering is built on trust, reliability, and the ability to adapt to every client's unique need.",
  points: [
    "Thoughtfully curated menus for every occasion",
    "Uncompromised quality and hygiene standards",
    "Experienced chefs and streamlined kitchen operations",
    "Fresh preparation with on-time delivery",
    "Seamless adaptation to every client's preferences",
  ],
  closing:
    "From everyday corporate dining to large-scale celebrations — we turn every service into a smooth, satisfying food experience.",
};

// ── 3. CULINARY SPECTRUM ──────────────────────────────────────────────────────
export const culinarySpectrum = [
  {
    id: 1,
    icon: "🥗",
    title: "Starters & Chaat Counter",
    image: "/images/gallery/danie-franco-7XqL_DVuBMw-unsplash.webp",
    description:
      "Gol Gappe with three types of water, Dahi Bhalla with Papri, Crispy Aloo Tikki with Chole, Paneer Tikka, Mushroom Tikka, Tandoori Aloo, Hara Bhara Kebab, Veg Seekh Kebab, Cocktail Samosa, Spring Rolls, French Fries, and more — all served with mint chutney and tomato ketchup.",
  },
  {
    id: 2,
    icon: "🍛",
    title: "Indian Vegetarian Mains",
    image: "/images/gallery/junior-reis-xMDo8y776uE-unsplash.webp",
    description:
      "Paneer Lababdar, Kadhai Paneer, Dal Makhani, Malai Kofta Nargisi, Mix Vegetable / Navratan Korma, Matar Mushroom, Pindi Chole, Dum Aloo, Palak Corn Paneer, Dal Tadka, Rajma Masala, Sarson ka Saag (seasonal), and more — paired with Jeera Rice, Veg Biryani, and freshly made breads.",
  },
  {
    id: 3,
    icon: "🌍",
    title: "Live Multi-Cuisine Counters",
    image: "/images/gallery/eugenia-pan-kiv-1Bs2sZ9fD2Q-unsplash.webp",
    description:
      "Stone-Fired Pizzeria (thin-crust, Farm House, Veggie Lover's), Italian Live Pasta Bar (Penne / Fusilli / Farfalle in Arrabiatta, Mushroom Creamy or Cheese sauce), Chinese Tadka (Hakka Noodles, Veg Manchurian, Fried Rice), South Indian (Dosa, Idli Sambhar, Medu Vada), Chandni Chowk Special (Bedmi Poori, Sitafal ki Sabji), and Agra ke Paranthe.",
  },
  {
    id: 4,
    icon: "🍨",
    title: "Desserts & Sweet Counters",
    image: "/images/gallery/pixel-studios-IFCN-tBVNPI-unsplash.webp",
    description:
      "Hot Serving: Gulab Jamun, Moong Dal ka Halwa, Gajar ka Halwa, Jalebi with Rabri, Mathura ka Kadhai Doodh. Cold Serving: Ras Malai, Raj Bhog, Gulab Kheer, Tila Kulfi, Kulfi Mania (Aam, Kesar, Pista, Anar, Cream), Kulfi Faluda, Ice Cream. Plus Ice Cream Parlour, Cake & Pastry Counter, and Pan Shoppe.",
  },
  {
    id: 5,
    icon: "🥤",
    title: "Beverages & Mocktail Bar",
    image: "/images/gallery/vadim-paripa-PuXtB1B4zL8-unsplash.webp",
    description:
      "Mocktail Bar: Vergin Mojito, Pina Colada, Blue Angle, Green Vally, Vanilla / Strawberry / Chocolate / Mango / Oreo / Chiku Shakes, Fresh Orange, Watermelon and Guava Juices, Sweet Lime, and Black Grapes. Hot Drinks: Masala Kullad Tea and Espresso. Coffee Parlour: Cappuccino, Mochaccino, Cold Coffee, Iced Tea, and Cookies.",
  },
  {
    id: 6,
    icon: "🫕",
    title: "Soups, Salads & Raitas",
    image: "/images/gallery/khadija-yousaf-lKwp3-FQomY-unsplash.webp",
    description:
      "Soups: Cream of Tomato, Vegetable Sweet Corn, Hot 'N' Sour, Manchow, Tamatar aur Dhaniya ka Shorba. Salads: Garden Green, Kachumbar, Russian, Sweet Corn, Beetroot, Mix Fruit, Aloo Chana Chaat, Pasta & Macaroni. Raitas: Boondi, Mix, Pineapple, Fruit, Burani, and Masala Chach.",
  },
];

// ── 4. EVENTS WE CATER ───────────────────────────────────────────────────────
export const eventsWeCater = [
  "Weddings & Receptions",
  "Pre-Wedding Functions",
  "Engagements & Ring Ceremonies",
  "Birthdays & Anniversaries",
  "Corporate Events & Conferences",
  "Sundowner Parties",
  "Cocktail Parties",
  "Private & House Parties",
  "Festive Events",
  "Religious Functions",
  "Kitty Parties & Social Gatherings",
  "Theme-Based Events",
  "Live Food Counters",
  "Outdoor & Destination Events",
  "Institutional & Campus Catering",
  "Office & Corporate Dining",
];

// ── 5. SERVICE EXCELLENCE ─────────────────────────────────────────────────────
export const serviceExcellence = [
  "Thoughtfully curated menus for every event scale",
  "Refined buffet presentation and elegant setup",
  "Experienced chefs and well-trained service staff",
  "Strict hygiene and quality-controlled preparation",
  "Timely delivery and seamless on-ground coordination",
  "Fully customizable menus to match client preferences",
  "Live food counters with fresh, on-spot preparation",
  "End-to-end event catering management",
];

// ── 6. GALLERY ────────────────────────────────────────────────────────────────
// Replace these paths with your actual uploaded gallery images
export const cateringGallery = [
  { id: 1, image: "/images/Catering/catering-1.png", alt: "Premium catering service at Basti Ram Palace" },
  { id: 2, image: "/images/Catering/catering-2.png", alt: "Elegant food presentation and buffet setup" },
  { id: 3, image: "/images/Catering/catering-3.png", alt: "Professional service staff and event hospitality" },
  { id: 4, image: "/images/Catering/catering-4.png", alt: "Customized culinary delights for special occasions" },
  { id: 5, image: "/images/Catering/catering-5.png", alt: "Freshly prepared snacks and starters" },
  { id: 6, image: "/images/Catering/catering-6.png", alt: "Exquisite main course selection and dining" },
  { id: 7, image: "/images/Catering/catering-7.png", alt: "Desserts and sweet treats for guests" },
];

// ── 7. TRUSTED CLIENTS ────────────────────────────────────────────────────────
export const trustedClients = [
  { initials: "IICA", name: "Indian Institute of Corporate Affairs" },
  { initials: "SG", name: "Signature Global" },
  { initials: "Lilly", name: "Eli Lilly" },
  { initials: "Vatika", name: "Vatika Group" },
  { initials: "EHS", name: "Eugenix Hair Sciences" },
  { initials: "II", name: "IndusInsights" },
  { initials: "SVDK", name: "Sandvik" },
  { initials: "MU", name: "Masters' Union" },
  { initials: "ACL", name: "Anchal" },
  { initials: "CB", name: "CoolBoots" },
  { initials: "UCM", name: "Unicosmos" },
  { initials: "HWI", name: "Huawei" },
  { initials: "NTR", name: "Nantara Farm House" },
  { initials: "TLF", name: "The Liquor Fort" },
];
